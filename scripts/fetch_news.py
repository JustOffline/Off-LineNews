#!/usr/bin/env python3
"""
BANNED Tracker — Daily News Fetcher
Pulls from free RSS feeds, filters by keywords, injects HTML cards into index.html.
...
MAX_ARTICLES        = 12      # cards shown on page
FRESHNESS_DAYS      = 30      # ignore articles older than this
DEDUP_FILE          = "scripts/seen_guids.json"  # tracks seen article IDs
...
def fetch_rss(url: str, source_name: str) -> list[dict]:
    ...
    headers = {
        "User-Agent": "BANNED-Tracker/1.0 (https://github.com; news aggregator for public interest research)",
...
def source_short(name: str) -> str:
    """Shorten long source names for the badge."""
    replacements = {
        "BBC Technology": "BBC",
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  BANNED Tracker — News Fetch — {last_run}")
    print(f"{'='*60}\n")
    ...
    # 6. Build HTML block
    html_block = render_block(selected, last_run)

    # 7. Inject into index.html
    if not index_path.exists():
        print(f"\n❌ index.html not found at {index_path}", file=sys.stderr)
        sys.exit(1)

    content = index_path.read_text(encoding="utf-8")
    pattern = r"<!-- NEWS_INJECT_START -->.*?<!-- NEWS_INJECT_END -->"
    new_content, count = re.subn(pattern, html_block, content, flags=re.DOTALL)

    if count == 0:
        print("\n❌ Inject markers not found in index.html", file=sys.stderr)
        sys.exit(1)

    # 8. Update "last updated" footer
    new_content = re.sub(
        r'Data compiled from public sources · [A-Za-z]+ \d{4}',
        f'Data compiled from public sources · {now_utc.strftime("%B %Y")} · Auto-updated daily',
        new_content
    )

    index_path.write_text(new_content, encoding="utf-8")
    print(f"\n✅ index.html updated — {len(selected)} cards injected")

    # 9. Update dedup cache (only with newly-selected articles to avoid locking out all fresh ones)
    for a in selected:
